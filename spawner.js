export class Spawner {
    constructor(currentPolys, maxPoly, maxSides, Polygon, globalPolygons, canvas, polygonColors) {
    ...
    spawn(chosenSides, x, y) {
        ...
        let poly = new this.Polygon(x, y, chosenSides, this.polygonColors);
    ...
    spawnLoop() {
        if (this.currentPolys < this.maxPoly) {
           ...
           for (let i = 0; i < 1 + Math.ceil(Math.random()*5); i++) {
            let chosenSides = 2 + Math.ceil(Math.random() * (10 - 2))
                this.spawn(
                    chosenSides,
                    randX,
                    randY
